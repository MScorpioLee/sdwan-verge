...
VIEW="$ROOT/files/www/luci-static/resources/view/sdwan-verge/dashboard.js"
...
assert_contains "option mode 'half_route'" "$CONFIG"
...
assert_contains "option tun_interface 'sdwan-tun0'" "$CONFIG"
assert_contains "option tun_backend '/usr/libexec/sdwan-verge/sdwan-verge-tun'" "$CONFIG"
assert_contains "list test_site 'Claude|https://claude.ai/'" "$CONFIG"
assert_contains "list test_site 'Amazon|https://www.amazon.com/'" "$CONFIG"
assert_contains "config_get MODE" "$CORE"
assert_contains "config_get TUN_INTERFACE" "$CORE"
assert_contains "config_get TUN_BACKEND" "$CORE"
assert_contains "ip route replace 0.0.0.0/1 via" "$CORE"
assert_contains "ip route replace 128.0.0.0/1 via" "$CORE"
assert_contains "tun_available" "$CORE"
assert_contains "traffic_json" "$CORE"
assert_contains "sites_json" "$CORE"
assert_contains "curl -4" "$CORE"
...
assert_contains "opkg remove luci-app-sdwan-verge" "$CORE"
assert_contains "sdwan-verge-core" "$INIT"
assert_contains "admin/services/sdwan-verge" "$MENU"
assert_contains "sdwan-verge/dashboard" "$MENU"
assert_contains "luci-app-sdwan-verge" "$ACL"
assert_contains "SD-WAN Verge" "$VIEW"
assert_contains "sdwan-panel-grid" "$VIEW"
assert_contains "Half Route" "$VIEW"
assert_contains "TUN" "$VIEW"
assert_contains "流量检测" "$VIEW"
assert_contains "常用网站检测" "$VIEW"
assert_contains "formatRate" "$VIEW"
assert_contains "卸载插件" "$VIEW"
