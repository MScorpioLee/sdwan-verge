...
class HelpPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    const items = [
      ...
      ('桌面端', 'macOS、Windows、Linux 使用各自的原生 TUN 后端，Flutter 只负责界面、配置和状态展示。'),
      (
        '手机端',
        'Android 使用 VpnService，iOS 使用 Network Extension Packet Tunnel，交互和桌面端保持一致。',
      ),
      (
        'OpenWrt/iStoreOS 插件',
        '插件安装在旁路由或 CPE 上，用于提供局域网侧入口和状态管理，不再要求客户端改物理网卡配置。',
      ),
    ];
